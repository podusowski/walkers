...
async fn anticipate_then_request() {
    ...
    let request = mock.anticipate("/foo");
    ...
    futures::future::join(
        ...
        async {
            request.respond(Bytes::from_static(b"hello"));
        },
    )
    .await;
}
...
async fn anticipate_expect_then_request() {
    ...
    let mock = Server::bind().await;
    let url = format!("http://localhost:{}/foo", mock.port());
    let mut request = mock.anticipate("/foo");
...
async fn respond_with_http_status() {
    ...
    let url = format!("http://localhost:{}/foo", mock.port());
    let request = mock.anticipate("/foo");
    request.respond_with_status(StatusCode::NOT_FOUND);
