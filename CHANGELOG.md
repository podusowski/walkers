...
* Vector tiles are now drawn using `wgpu`. This means that `wgpu` needs to be turn on in the
  `egui`. `walkers::install_renderer` needs to be called on startup. Raster tiles work as before.
